<!-- ASCEND — white-label. No personal or company identifiers in this file by design. -->

<p align="center">
  <img src="assets/hero.svg" alt="ASCEND" width="100%">
</p>

<h1 align="center">🛰️ ASCEND</h1>
...
node lib/ascend.cjs --once            # run one cycle over the example tasks (shell runner)

# 2. loop forever on an interval (run as a service)
node lib/ascend.cjs --loop --interval=1800
...
ascend/
├── lib/
│   └── ascend.cjs           ← the loop: UCB1 select → run → record, pid-liveness lock
├── examples/
...
2. **Bring your own executor.** The runner is one function; ASCEND owns the loop, the lock, and the recording, not what a task does.
...
<p align="center"><sub>ASCEND · select · run · record · MIT</sub></p>
