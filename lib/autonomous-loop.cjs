...
'use strict';
/*
 * autonomous-loop.cjs — a crash-safe autonomous task-runner loop.
 *
...
const DATA = process.env.AUTONOMOUS_LOOP_DATA_DIR ? path.resolve(process.env.AUTONOMOUS_LOOP_DATA_DIR) : path.join(ROOT, 'data');
const STATS = path.join(DATA, 'stats.json');
const LOG = path.join(DATA, 'log.jsonl');
const LOCK = path.join(DATA, 'autonomous-loop.lock');
...
if (require.main === module) {
  ...
  const tasksFile = OPT('tasks', process.env.AUTONOMOUS_LOOP_TASKS || path.join(ROOT, 'examples', 'tasks.example.json'));
  ...
  const opts = {
    ...
    onCycle: (r) => console.log(`[autonomous-loop] task=${r.id} ok=${r.ok} reward=${r.reward}${r.plays ? ` plays=${r.plays} avg=${r.avgReward}` : ''}`),
  };
  ...
  (async () => {
    ...
    if (!out.started) console.log('[autonomous-loop] ' + out.reason);
